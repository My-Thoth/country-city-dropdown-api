...
def seed_countries():
    ...
    {"code": "ZW", "name": "Zimbabwe", "region": "Africa"},
        ...
        # Load the CSV file
        df = pd.read_csv(r'C:\Users\USER\Desktop\api\cities.csv')

        # Initialize an empty dictionary to store country codes and their cities
        cities_data = {}

        # Iterate through the rows of the dataframe to populate the cities_data dictionary
        for index, row in df.iterrows():
            country_code = row['country_code']
            city_name = row['state_name']  # state_name is the city in the sample CSV

            # If the country_code is not in cities_data, add it with an empty list
            if country_code not in cities_data:
                cities_data[country_code] = []
            
            # Add the city to the list of cities for the given country_code
            cities_data[country_code].append(city_name.strip())
